...
class DbSessionMiddleware(BaseMiddleware):
    """
    DbSessionMiddleware відповідає за створення та керування сесією БД
    для кожного повідомлення, яке приходить від Telegram.

    1. aiogram отримує update від Telegram
    2. Перед тим як викликати handler — виконується __call__
    3. Відкриваємо нову сесію і кладемо в data["session"]
    4. Handler отримує session як параметр (magic injection)
    5. Після handler — сесія автоматично закривається

   Middleware — це aiogram-спосіб
